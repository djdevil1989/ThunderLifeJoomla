...
function init_SearchDataUpdate($parameter) {
	global $debug;
	if(is_array($debug) && array_key_exists("search",$debug) && $debug['search'] == "yes") {
		$debugout = var_export($parameter,TRUE);
...
function parseData($host,$port) {
	global $debug;
//	global $params;
//	$jopensim_expire = $params->get('searchupdateinterval');
	$jopensim_expire = 0;
	$db =& JFactory::getDBO();
	...
	foreach($regionlist as $region) {
		...
		if(is_array($regionData) && array_key_exists("locX",$regionData) && array_key_exists("locX",$regionData)) {
...
//		$parceldata = $data->getElementsByTagName("parceldata");
		$parcel = $data->getElementsByTagName("parcel");
		foreach($parcel as $value) {

			if(is_array($debug) && array_key_exists("search",$debug) && $debug['search'] == "yes") {
				$debugout = var_export($value,TRUE);
...
function dir_classified_query($parameter) {
	...
	if(!is_array($parameter) || !array_key_exists("text",$parameter) || !array_key_exists("category",$parameter) || !array_key_exists("query_start",$parameter) || !array_key_exists("flags",$parameter) || $parameter['text'] == "%%%") {
		...
		if($pgonly > 0 && $matureonly == 0) { // mature or adult classified should not be included
			$flagwhere = " AND #__opensim_userclassifieds.classifiedflags & 72 = 0";
		} elseif($pgonly == 0 && $matureonly > 0) {
			$flagwhere = " AND #__opensim_userclassifieds.classifiedflags & 72 > 0";
		} else {
			$flagwhere = "";
		}
		...
		if($category != 0) { // search for special category
			$categorywhere = sprintf(" AND #__opensim_userclassifieds.category = '%d'",$category);
		} else {
			$categorywhere = "";
		}

		$db =& JFactory::getDBO();
		$query = sprintf("SELECT #__opensim_userclassifieds.* FROM #__opensim_userclassifieds
							WHERE (#__opensim_userclassifieds.name LIKE '%%%1\$s%%' OR #__opensim_userclassifieds.description LIKE '%%%1\$s%%')
							%2\$s
							%3\$s
							ORDER BY
								#__opensim_userclassifieds.priceforlisting DESC,
								#__opensim_userclassifieds.name ASC
							LIMIT
								%4\$s,101",
				$text,
				$flagwhere,
...
function classifieds_info_query($parameter) {
	...
	if(!is_array($parameter) || !array_key_exists("classifiedID",$parameter)) {
		...
		if($num_rows == 1) {
			...
			$retval['data'] = array();
//			$retval['success'] = FALSE;
//			$retval['errorMessage'] = "Error while fetching data for classified ".$classifiedID;
		}
	}
	return $retval;
}
